...
def get_hypervolume(predicted_pareto_front,ref, alg):
    ...
    ind = HV(ref_point= -ref[0, :])
    hypv  = ind(predicted_pareto_front_minus)
    return hypv
...
def plot_pareto_front(pred_front, true_front, evaluations, func_name):
    ...
    ax.grid( which='major', color='black', alpha=0.5, linestyle='--', linewidth=0.5)
...
def plot_hypervolume(path, num_eval, eps_perc, iteration, func_name):
    from experiment_setup import objective_function
    ...
    hypv_list = list()
    time_list = np.arange(1, num_eval)
    for eval in range(num_eval-1):
        with h5py.File(path, 'r') as hf:
            adaptive_pareto_front = np.array(hf.get('epsilon_percent_{}_iter_{}_eval_{}_pareto_front'.format(eps_perc,
                                                                                                 iteration, eval+1)))
            hypv = get_hypervolume(adaptive_pareto_front, ref, alg = 'adapt')
            hypv_list.append(hypv)


    fig, ax = plt.subplots(1)
    ax.scatter(time_list, hypv_list, label='Hypervolume', alpha=0.9, marker="o", color='red', s=9)
    ...
    ax.tick_params(axis='x', labelsize=12)
    ax.tick_params(axis='y', labelsize=12)
    ax.set_title(f'hypervolume vs. evaluations')

    plt.savefig(os.path.join('Results', func_name, 'hypervolume_vs_time.pdf'))
    plt.close()